...
def layout():
    ...
    if 'loggedin' in session:
        ...
        return render_template('home.html', account=account)
             ...
             return render_template('login.html', msg=msg), 404
...
def Videos():
    ...
    if 'loggedin' in session:
        ...
        return render_template('videos.html', row=rows)
    else:
        return render_template('login.html', msg=msg), 404
    


@app.route('/views_videos/<id>', methods=['GET', 'POST'])
def views_videos(id):
    msg=''
    ...
    if 'loggedin' in session:
        ...
        conn = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        sql = ('SELECT url, titulo FROM videos where id=%s')
        args=[id]
        ...
        return render_template('views_videos.html', row=rows)
    else:
        return render_template('login.html', msg=msg), 404
...
def upload_videos():
    ...
    if 'loggedin' in session:
        if request.method == 'POST':
            msg='Archivo ingresado'
            archivo = request.form['url']
            titulo = request.form['titulo']
               
            url= archivo.split('=')[-1]
            results =url2+''+ url
            print('Link',results)
            query = "INSERT INTO videos (url, titulo) VALUES (%s, %s)"
            values = (results, titulo)
            conn.execute(query, values)
            mysql.connection.commit()
            conn.close()
            conn.close()
        ...
        # sql = ('SELECT url, titulo FROM videos')
        # conn.execute(sql)
        # rows = conn.fetchall()
        # for row in rows:
        #     url = row['url']
        #     titulo = row['titulo']
        #     results =url2+''+ url.split('=')[-1]
        #     print(row)
       
    
            
        return render_template('upload_videos.html', row=msg)
    else:
        return render_template('login.html', msg=msg), 404
